### Example of using selinux troubleshooting tools.  It didn't work, but that ok.
```
...
### The container runs in a isolated network namespace and can't access the host network to run the scan, so it fails.
...
```
```
### Checking sealert, remember it's a tools to help troubleshoot SElinux issues.
